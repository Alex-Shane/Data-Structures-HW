...
Results for moby dick:
    arrayMap = 2978.226 ms/op
    avlTreeMap = 101.893 ms/op
    bstMap = 103.280 ms/op
    treapMap = 120.404 ms/op

Looking at the first two datasets tested (which are much shorter than the last 2), we see that arrayMap performed the worst,
which makes sense as its time complexity is O(n) for core operations. Although the balanced trees performed worse than BST,
it was only by a small amount and can be explained by looking at the small sample size of these inputs. BST has a faster
insertion than the balanced trees since it doesn't balance itself but a slower find() method of O(n) time, however, since
the data set is so small it makes sense that BST could perform better as O(lgn) vs. O(n) isn't drastic for small sets. Further,
it makes sense that AVL performed better than Treap because treap becomes more efficient and easier to claim that it performs at 
O(lgn) as data gets infinitly large, so it makes sense that it would be more inefficient on a smaller set due to the randomness
that its operations are dependent on. 

Looking at the last two datasets (much longer than first 2), we again see that arrayMap was far and away the worst performing 
map and this makes sense as with a large set, we can see how much more inefficient O(n) is compared to O(lgn). Further,
we see that AVL was the best performing map, followed by bstMap and then Treap for both sets. It makes sense that AVL would
outperform BST in a large set because its find algorithm is faster unless BST was perfectly balanced which isn't likely on a
large set such as we tested. It also makes sense that BST could outperform Treap because treap is not for sure always going
to perform at O(lgn) (could be worse), our BST could've been close to its best case of O(lgn) efficiency, and BST doesn't 
expend time to perform rotations to balance itself. 
